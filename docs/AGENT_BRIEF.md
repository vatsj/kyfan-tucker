> **Historical document.** This is the working brief under which the computations in this repository were carried out,
> kept verbatim for the record. It is not the entry point: start from `README.md` and `docs/README.md`. File names have moved
> since it was written: `results.md` is now `HISTORY.md`; `review.md` is `analysis/legacy/review_scripts.md`;
> `TREE_GADGET_THEOREM.md`, `REALIZATION_THEOREM.md`, `SIZE_LOWER_BOUND.md` are `docs/tree_gadget.md`, `docs/realization.md`,
> `docs/size_lower_bound.md`; the tower and Ky Fan proofs of §2 are written out in `docs/tower.md` and `docs/kyfan_lower_bound.md`.
